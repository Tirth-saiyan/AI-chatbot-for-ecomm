# AI-Chatbot Project Setup

This guide provides step-by-step instructions on setting up AI-chatbot project on Google Cloud Platform.
...
```bash
git clone https://github.com/Tirth-saiyan/AI-Chatbot-for-ecomm/tree/dev
cd AI-Chatbot-for-ecomm
```
...
2. Open the `.env` file in your favorite text editor and add your API keys in the following format:
    ...
    ```
Make sure: `HOST` is set to "0.0.0.0" and `PORT` is set to 80. Generate a unique uuid token for `AI_API_ACCESS_TOKEN` or assign it any string value.
...
6. Save the file name as `AI-gcp.json` and set `GOOGLE_APPLICATION_CREDENTIALS` in `.env` as `./AI-gcp.json`
...
cd data_pipeline
```
and follow the instructions provided in the [readme.md](https://github.com/Tirth-saiyan/AI-Chatbot-for-ecomm/blob/dev/Data_Pipeline/README.md) file.
...
- **Test Data Ingestion**: This step takes approximately **2 hours** to complete. However, the test data ingestion is already present, so you can skip this step if desired.  
...
You can read the [API_README.md](https://github.com/Tirth-saiyan/AI-Chatbot-for-ecomm/blob/dev/readme/API_README.md) file (located in the same folder) for details about the different endpoints the app exposes. To test the endpoints, visit `https://0.0.0.0:80/docs` in your browser, or use Postman to make requests to the API.
...
1. **Create Docker Artifact in Artifact Registry**  
   ...
   - Create a new **Docker Artifact** and name the registry as `AI-chat`.
...
2. **Set Secrets in GitHub Repository**  
   ...
   - In the **Secrets** tab, set up the following secrets:
     ...
     - **GAR_NAME**: `AI-chat` (name of the Artifact Registry).
     - **GCP_SA_KEY**: Copy and paste the content of your `AI-gcp.json` (service account JSON key) into this secret.
     ...
     - **SERVICE**: `AI-chat-service` (or choose a unique service name for Cloud Run).
